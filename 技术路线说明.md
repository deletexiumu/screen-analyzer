...
│   ├── Cargo.toml
│   ├── tauri.conf.json
│   └── vendor/             # 本地重写的第三方依赖（例如 claude-agent-sdk 超时补丁）
├── src/                    # 前端
│   ├── App.vue
│   ├── components/
│   │   ├── Calendar.vue
...
> 说明：`src-tauri/vendor/claude-agent-sdk` 存放带有自定义流超时控制的 Claude Agent SDK，本地依赖通过 `Cargo.toml` 指向该目录，以避免上游默认 30 秒限制导致的长任务超时。
